...
7. Record `.mamkin/template-version.json` baseline metadata when the copied template commit is known; otherwise leave it `TBD` for first-sync review mode.
8. When init is complete, hand off to the coordinator using the packet shape in `docs/process/init-agent.md`.
